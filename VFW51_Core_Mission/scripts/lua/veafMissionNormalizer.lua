...
function veafMissionNormalizer.normalizeMission(filePath)
  ...
  -- VFW51 addition
  local resourceKeysThatAreActuallyUsed = {}
  local WHATS_IN_A_RESOURCE_KEY = "ResKey_"
  local function _recursivelySearchForResourceKeysInTable(t, resourceKeys)
    for key, value in pairs(t) do
      if type(value) == "table" then
        _recursivelySearchForResourceKeysInTable(value, resourceKeys)
      elseif type(value) == "string" then
        if value:lower():sub(1, #WHATS_IN_A_RESOURCE_KEY) == WHATS_IN_A_RESOURCE_KEY:lower()  then
            resourceKeys[value:lower()] = value
        end
      end
    end
  end
  ...
  local function _processFunctionForMission(t)
    ...
    _recursivelySearchForResourceKeysInTable(t, resourceKeysThatAreActuallyUsed)
    _sortTable(t)
    return t
  end
  ...
  -- VFW51 Addition
  local function _processFunctionForResources(t)
    -- only keep keys that have been referenced in resourceKeysThatAreActuallyUsed 
    local result = {}
    local nSkippedKeys = 0
    for key, value in pairs(t) do
      if resourceKeysThatAreActuallyUsed[key:lower()] then
        result[key] = value
      else
        veafMissionNormalizer.logDebug(string.format("removing unused resource key [%s]=%s" , key, tostring(value)))
        nSkippedKeys = nSkippedKeys + 1
      end
    end
    if nSkippedKeys > 0 then 
      veafMissionNormalizer.logInfo(string.format("removed %d unused keys from mapResource" , nSkippedKeys))
    end
  ...
  veafMissionEditor.editMission(_filePath, _filePath, "mapResource", _processFunctionForResources)
end
