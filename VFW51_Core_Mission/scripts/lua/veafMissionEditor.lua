...
function veafMissionEditor.serialize(name, value, level)
  ...
  local serialize_to_t = function(name, value, level)
      ...
      elseif type(value) == "table" then
          ...
          veafMissionEditor.logError("Cannot serialize a " .. type(value) .. " " .. name)
      end
      return var_str_tbl
  end
...
function veafMissionEditor.readMissionFile(filePath, tableName)
    ...
    file()
    -- loadstring no longer a thing in lua 5.2
    -- returner = loadstring("return "..tableName)
---@diagnostic disable-next-line: param-type-mismatch
    local returner = load("return " .. tableName)
    assert(returner)
...
function veafMissionEditor.processMission(mission, arg)
...
function veafMissionEditor.editMission(inFilePath, outFilePath, tableName, processFunction, arg)
    ...
    _table = _processFunction(_table, arg)
