-- ************************************************************************************************************
--
-- vfw51_mission_splash_damage.lua: Splash damage script
--
-- ************************************************************************************************************

env.info("*** Loading Mission Script: vfw51_mission_splash_damage.lua")
...
local function explodeObject(params)
    trigger.action.explosion(params[1], params[3])
end
...
local function modelUnitDamage(units)
...
local function onWpnEvent(event)
    if event.weapon and event.id == world.event.S_EVENT_SHOT then
        ...
        if (weapon_desc.category ~= 0) and event.initiator then
            if (weapon_desc.category == 1) then
                if (weapon_desc.MissileCategory ~= 1 and weapon_desc.MissileCategory ~= 2) then
                    tracked_weapons[event.weapon.id_] = { wpn = ordnance,
